...
class FormattingOptions:
    ...
    def __init__(
        ...
        self.hide_recursive_layers = RowSettings.HIDE_RECURSIVE_LAYERS in self.row_settings
    ...
    def get_start_str(self, depth: int) -> str:
        ...
        return ("|    " if self.ascii_only else "│    ") * (depth - 1) + ("+ " if self.ascii_only else "└─")
    ...
    def layer_info_to_row(self, layer_info: LayerInfo, reached_max_depth: bool, total_params: int, total_macs: int) -> str:
        ...
        values_for_row = {
            ...
            ColumnSettings.PARAMS_PERCENT: layer_info.params_percent(total_params, reached_max_depth),
    ...
    def layers_to_str(self, summary_list: list[LayerInfo], total_params: int, total_macs: int) -> str:
        ...
        for layer_info in summary_list:
            if layer_info.depth > self.max_depth or self.hide_recursive_layers and layer_info.is_recursive:
                continue

            reached_max_depth = layer_info.depth == self.max_depth
            new_str += self.layer_info_to_row(layer_info, reached_max_depth, total_params, total_macs)
        return new_str
